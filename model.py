...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        # Projection of key, query, value for all heads in a batch
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.gelu = nn.GELU(approximate='tanh') # Approximation for historic reasons
...
class GPT(nn.Module):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            torch.nn.init.normal_(module.weight, mean=0.0, std=0.02) # Word embedding will be initialized twice
